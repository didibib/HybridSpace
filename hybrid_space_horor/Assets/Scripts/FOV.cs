...
public class FOV : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        StartCoroutine("FindTargetsWithDelay", .2f); // every .2 of a second the script will check for targets;
    }
    ...
    void LateUpdate() // call after the controller has updated, to fix the jitter;
    {
        DrawFieldOfView();
    }
    ...
    public Vector3 DirFromAngle(float angleInDegrees, bool angleIsGlobal)
    {
        if (!angleIsGlobal)
        {
            angleInDegrees += transform.eulerAngles.y; // the field of view will turn with the rotation of the object
        }
        return new Vector3(Mathf.Sin(angleInDegrees * Mathf.Deg2Rad), 0, Mathf.Cos(angleInDegrees * Mathf.Deg2Rad)); // calculate field of view
    }
